...
const (
	dockerImageCpp   = "axope/sandbox:judger-cpp-0.1"
	containerNameCpp = "JOJ-cpp"
)

func checkRequiredFilesCpp() bool {
...
func judgeSolutionByCppFromProtobuf(req *pb.Judge) (pb.StatusSet, int64, int64, error) {
	...
	if !checkRequiredFilesCpp() {
		return pb.StatusSet_UKE, -1, -1, nil
	}
	
	containerID, err := createAndRunContainer(dockerImageCpp, containerNameCpp)
	if err != nil {
